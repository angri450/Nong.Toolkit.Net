# Nong.Toolkit.Net 插件分发方案评估

Date: 2026-06-04

## 问题背景

Nong.Toolkit.Net 是纯 .NET 项目（19 个 Claude Code skills），目前通过 Git 仓库分发。普通用户（国内、农学背景）安装时会遇到以下障碍。
...
```powershell
claude plugin marketplace add https://gitee.com/angri450/Nong.Toolkit.Net.git
```
...
问题：Nong.Toolkit.Net 是纯 .NET 项目，不应该为 Claude Code 插件分发引入 npm 技术栈。维护 package.json、npm publish 流程与 .NET 工具链不一致，增加维护负担。

### 5. Gitee raw marketplace.json + source: "./"

```powershell
claude plugin marketplace add https://gitee.com/angri450/Nong.Toolkit.Net/raw/main/.claude-plugin/marketplace.json
```
...
```powershell
irm https://gitee.com/angri450/Nong.Toolkit.Net/raw/main/install.ps1 | iex
```
...
2. 解压到稳定本地目录 `%LOCALAPPDATA%\Nong.Toolkit.Net`。
...
8. 执行 `claude plugin install --scope user nong-toolkit@angri450`。
...
3. 解压到固定目录（如 `$env:USERPROFILE\.nong-toolkit`）
4. 执行 `claude plugin marketplace remove angri450`（清理旧版）
5. 执行 `claude plugin marketplace add $env:USERPROFILE\.nong-toolkit`
6. 执行 `claude plugin install nong-toolkit@angri450`
